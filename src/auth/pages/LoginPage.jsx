...
export const LoginPage = () => {
  ...
  return (
    <div className='w-full h-[90vh] box-border flex flex-col justify-center items-center relative  overflow-hidden'>
      <form className='w-11/12 flex flex-col gap-5
                       lg:w-96'>
        {/* INPUTS */}
        ...
        <div className='Inputs w-full flex flex-col gap-5'>
          ...
          <div className='w-full flex flex-row relative items-center'>
            ...
            />
            <button onClick={() => setViewPass(!viewPass)} type='button' className='absolute w-12 h-12 right-0 flex items-center justify-center'>
              {
        ...
        <div className='Buttons flex flex-col'>
          <button type='submit' className='h-12 bg-gray-200 rounded text-gray-300'>Iniciar Sesión</button>
          <div className='flex flex-col gap-4 items-center mt-4'>
            <Link to={'/'} className='text-sm text-gray-500'>¿Se te olvido la contraseña?</Link>
            <Link
              to={'/register'}
              className='text-sm text-gray-500'>¿No tienes una cuenta?</Link>
          </div>
        </div>
      </form>
    </div>
